...
- (void)testOpen{
    ...
    [query getFirstObjectInBackgroundWithBlock:^(AVObject * _Nullable object, NSError * _Nullable error) {
        if ([object[@"app"] isEqualToString:@"1"]) {
...
- (void)pushWebView:(NSString *)url{
    FYWebVC *webVC = [[FYWebVC alloc] init];
    webVC.URL = url;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 100) {
        ...
        if (indexPath.section == 0) {
            ...
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"开发者" message:@"联系邮箱：wang8204375@gmail.com" preferredStyle:UIAlertControllerStyleAlert];
